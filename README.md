...
这些 skill 可以单独使用，也可以组成完整研究链路：先从赛道层面筛选值得研究的环节，再对具体公司做护城河分析，然后验证当前股价隐含预期，最后沉淀为统一格式的 Markdown 报告。也可以直接使用 `equity-research-pipeline` 一条指令跑完公司研究全流程；当已经生成多家公司报告后，再用 `equity-comparison-advisor` 做横向比较、排序和配置决策辅助。

```mermaid
flowchart TD
    S["sector-research<br/>行业与赛道研究"] --> A["company-moat-research<br/>公司护城河研究"]
    S --> P["equity-comparison-advisor<br/>股票横向比较顾问"]
    A --> B["valuation-expectation-check<br/>股价预期验证"]
    A --> C["integrated-equity-research-report<br/>综合股票研究报告"]
    B --> C
    P2["equity-research-pipeline<br/>一站式股票研究"] --> A
    C --> G["equity-comparison-advisor<br/>股票横向比较顾问"]

    S -.-> I["产业链 / 竞争格局 / 周期位置 / 投资机会"]
