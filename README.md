...
这些 skill 可以单独使用，也可以组成完整研究链路：先从赛道层面筛选值得研究的环节，再生成单家公司研究报告，最后对多家公司做横向比较。

`company-moat-research`、`valuation-expectation-check`、`integrated-equity-research-report` 三个 skill 既可以独立使用，也被 `equity-research-pipeline` 内部串联调用——用户只需提供公司名称即可一键跑完三步。

```mermaid
flowchart LR
    S["sector-research<br/>行业与赛道研究"] --> |"识别值得研究的公司"| P["equity-research-pipeline<br/>一站式股票研究"]
    P --> |"生成单家公司报告"| G["equity-comparison-advisor<br/>股票横向比较顾问"]
